...
Domain size is inferred automatically from the geometry file. A buffer of `--buffer`
lattice units is prepended and appended along x for inlet/outlet planes.
...
class PorousMedia(MultiphaseBGK):
    ...
    def set_boundary_conditions(self):
        if simulation == "imbibition":
            ...
            self.BCs[0].append(BounceBack(tuple(idx.T), self.gridInfo, self.precisionPolicy, theta_w[tuple(idx.T)], phi_w[tuple(idx.T)], delta_rho_w[tuple(idx.T)]))
            self.BCs[1].append(BounceBack(tuple(wall.T), self.gridInfo, self.precisionPolicy))
            self.BCs[1].append(BounceBack(tuple(idx.T), self.gridInfo, self.precisionPolicy, theta_a[tuple(idx.T)], phi_a[tuple(idx.T)], delta_rho_a[tuple(idx.T)]))
        else:
            inlet = self.boundingBoxIndices["left"]
    ...
    def output_data(self, **kwargs):
        ...
        S = v_w / (v_w + v_nw)
        file.write(f"{P_c},{S}\n")
        print(f"[t={timestep}] Pc={P_c:.6f}  S={S:.4f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BGK SWCC — water/air Shan-Chen LBM (arbitrary geometry size)")
    ...
    # -------------------------------------------------------------------------
    # Physics parameters (calibrated, scale-independent)
    # -------------------------------------------------------------------------
    precision = "f32/f32"
